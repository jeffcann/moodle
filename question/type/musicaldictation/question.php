...
class qtype_musicaldictation_question
    ...
    {
        print("<pre>This is some feedback");
        print_r($qa);
        print("</pre>");
        return parent::format_generalfeedback($qa); // TODO: Change the autogenerated stub
    }
    ...
    public function compare_response_with_answer(array $responsejson, question_answer $answerjson)
    {
        ...
        $response = json_decode($responsejson['answer'], false);
        ...
        foreach ($answer->staves as $staveidx => $stave) {
            foreach($stave->voices as $voiceidx => $voice) {
                foreach($voice->notes as $noteidx => $note) {
                    ...
                    if(!$responsenote->bar && strpos($responsenote->duration, 'r') !== false && $responsenote->keys[0] != $note->keys[0]) {
                        return false;
                    }
                }
            }
